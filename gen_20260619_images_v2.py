...
def generate_image_cogview(prompt_text, output_path):
    ...
    try:
        ...
        print(f"  Raw content type: {type(content)}, value: {str(content)[:200]}")
        
        # Handle list format returned by API
        image_url = None
        if isinstance(content, list):
            # API returned a list with URL objects
            for item in content:
        ...
        if image_url:
            ...
            print(f"  Downloading from URL: {image_url[:80]}...")
        ...
        print(f"  Unknown content format: {str(content)[:300]}")
        return False
...
def main():
    ...
    # Save news items
    with open("/tmp/news_items_20260619.json", "w", encoding="utf-8") as f:
