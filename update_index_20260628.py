...
content = re.sub(r'<title>.*?环球新闻</title>', f'<title>{today} 环球新闻</title>', content)
content = re.sub(r'<meta name="description" content=".*?">', f'<meta name="description" content="{today}全球20条热点新闻，涵盖科技、政治、军事、经济等领域的最新动态">', content)
content = re.sub(r'全球20条热点新闻 · \d{4}年\d{2}月\d{2}日', f'全球20条热点新闻 · {today}', content)

# Find and replace the news grid section
# Pattern to match the entire news-grid div
